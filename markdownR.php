...
function ou_markdownR_activate() {
  // Checks for the options of markdownR. If found do nothing else create one.\
	...
	add_option( 'ou_markdownR_options_magic_quotes', "0", "", "yes");
}
...
function ou_markdownR_deactivate() {
  // Deletes the values saved in the database. \
    ...
    delete_option( 'ou_markdownR_options_magic_quotes');
}
...
if( is_admin() ){
	...
	function ou_markdownR_options_page() {
		...
		add_action( 'admin_init', 'register_markdownr_plugin_settings' );
	}
}

function register_markdownr_plugin_settings() {
	//register our settings
	register_setting( 'markdownr-settings-group', 'ou_markdownR_options_R_path' );
	register_setting( 'markdownr-settings-group', 'ou_markdownR_options_rstudio_pandoc_path' );
	register_setting( 'markdownr-settings-group', 'ou_markdownR_options_magic_quotes' );
}
...
?>
<div class="wrap">
<h1>markdownR Options</h1>

<form method="post" action="options.php">
    <?php settings_fields( 'markdownr-settings-group' ); ?>
    <?php do_settings_sections( 'markdownr-settings-group' ); ?>
    <table class="form-table">
        <tr valign="top">
        <th scope="row">R path</th>
        <td><input type="text" name="ou_markdownR_options_R_path" value="<?php echo esc_attr( get_option('ou_markdownR_options_R_path') ); ?>" />(ex. /usr/bin/R)</td>
        </tr>
         
        <tr valign="top">
        <th scope="row">pandoc path</th>
        <td><input type="text" name="ou_markdownR_options_rstudio_pandoc_path" value="<?php echo esc_attr( get_option('ou_markdownR_options_rstudio_pandoc_path') ); ?>" />(ex. /usr/local/bin/pandoc)</td>
        </tr>
                 
        <tr valign="top">
        <th scope="row">magic quotes</th>
        <td><input type="text" name="ou_markdownR_options_magic_quotes" value="<?php echo esc_attr( get_option('ou_markdownR_options_magic_quotes') ); ?>" />(ex. 1)</td>
        </tr>
    </table>
...
function ou_markdownR_content( $content ) {
	if(preg_match("/^==Rmd==/", $content)){
		...
		$fp = fopen($temp, 'w');
		if(get_option("ou_markdownR_options_magic_quotes")){
			fwrite($fp, stripslashes($code));
		}else{
			fwrite($fp, $code);
		}
		fclose($fp);
